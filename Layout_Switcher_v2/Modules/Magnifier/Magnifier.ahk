...
SET_DEFAULTS:
{
	...
	defaults.zoom := 2
	defaults.follow := 1
	defaults.width := 32*12 ;420 ;350
	defaults.height := 32*9 ;240 ;250
...
{ ; AutoHotkey script "Screen Magnifier" - //autoHotkey.com/board/topic/10660-screenmagnifier/?p=456256
	...
	wwMin := ww-32*5 ;Round(ww * 0.5)
	whMin := wh-32*5 ;Round(wh * 0.5)
...
size_encrease:
{
	wwD := 32 ;Round(ww * 0.25) ;32
	whD := 32 ;Round(wh * 0.25) ;32
	gosub, Repaint
	IniWrite("ww", config_file, "Params", ww)
	IniWrite("wh", config_file, "Params", wh)
	return
}
; +WheelUp:: ; smaller
size_decrease:
