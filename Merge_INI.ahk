...
Create_GUI:
{
	...
	; GuiControl +AltSubmit, Method ; возвращает выбранную позицию (начина€ с 1) в списке вместо текста
...
FileReadSection(ByRef SourceFile, ByRef StartString, ByRef EndString = "", ByRef SkipComments := 1, ByRef CommentPattern := "^(\s+)?;", ByRef SkipEmptyLines := 1)
{
	...
	SectionContains := RegExReplace(SectionContains, "\s+$", "") ; удал€ем последние пустые строки (на вс€кий случай)
...
{ ; замена стандартного IniRead
			...
			{ ; поиск нужного ключа в указанной секции
				...
				if RegExMatch(LineText, "^(.*?)[ \t]?+=[ \t]?+(.*)$", Match) { ; найден какой-то ключ со значением
					...
					if (KeyName = Key) { ; найден ключ, который мы искали
						; return KeyValue ? KeyValue : KeyName ; возвращаем значение ключа, если оно есть, если значени€ нет - возвращаем сам ключ
				...
				if (LineText = Key) { ; найден ключ без знака равенства (обычный текст), сопадающий с искомым ключем
					...
					return LineText ; возвращаем ключ в виде текста (без знака равенства и значени€)
				}
			}
	...
	else { ; секци€ не была указана
		SectionsList := "" ; задаем "пустое" значени€ дл€ будущего списка секций
		Loop, Read, %Filename%
		{ ; ищем в файле "секции", использу€ шаблон RegEx
			if RegExMatch(Trim(A_LoopReadLine), "^\[(.*?)\]$", Match) { ; по шаблону найдена секци€
				SectionName := Match1 ; получаем им€ секции
				SectionsList .= SectionName . "`n" ; добавл€ем им€ в список секций
			}
		}
		SectionsList := RegExReplace(SectionsList, "\s+$", "") ; удал€ем последние пустые строки (на вс€кий случай)
...
MergeINI(ByRef ini_merge_to, ByRef ini_merge_from, ByRef ini_result := "_merge_ini_file3.ini", ByRef method := 1, ByRef progress := "")
{
	...
	if FileExist(file2)
	{
		if (method = 1) {
			...
			if InStr(FileExist(file3), "R") { ; перед обработкой сн€ть аттрибут "только чтение"
