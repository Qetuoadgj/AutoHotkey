...
if not %0%
{
	...
	IfMsgBox, No
			Exit  ; User pressed the "No" button.
	...
	IfMsgBox, Timeout
			Exit ; i.e. Assume "No" if it timed out.
	; Otherwise, continue:
	exit
}
...
IniRead,Name,%SourceFile%,Description,Name,false ; GetValue(SourceFile, "^Name[\s+]?=[\s+]?(.*)") ; Имя
; Name := Trim(Name, " " . "`t" . """")
...
IniRead,CreateNewArchives,%SourceFile%,Description,CreateNewArchives,false ; Создавать новый архив вместо синхронизации
